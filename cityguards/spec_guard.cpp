bool spec_guard(NPCharacter *ch)
{
	...
	for (victim = ch->in_room->people; victim != 0; victim = v_next)
	{
	   ...
	   */
		if (IS_SET(ch->in_room->area->area_flag, AREA_HOMETOWN) && number_percent() < 2 && !victim->is_immortal())
		{
	   ...
	   */
		if (!victim->is_npc() && (IS_SET(victim->act, PLR_WANTED) || victim->isAffected(gsn_jail)))
		{
			crime = "CRIMINAL";
			break;
		}
	   ...
	   */
		if (victim->is_npc() && victim->getNPC()->behavior && victim->getNPC()->behavior->hasDestiny())
			continue;
	   ...
	   */
		if (victim->fighting->is_npc() && victim->fighting->getNPC()->behavior && victim->fighting->getNPC()->behavior->hasDestiny())
			continue;
	...
	if (victim != 0)
	{
		...
		if ((ch->getModifyLevel() + 8 > victim->getModifyLevel()) && !is_safe_nomessage(ch, victim))
		{
